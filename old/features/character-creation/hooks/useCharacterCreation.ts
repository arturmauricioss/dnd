...
import type { CharacterStep } from '@features/character-creation/types'
import { gerarAtributos4d6, executarRegras, metodoLabel } from '@systems/attributes/rules/atributos'
import { calcularCusto } from '@systems/attributes/engine/atributos'
import { getModificadoresRaciais } from '@systems/races/rules/racas'
import { getDivindadesOrdenadas } from '@systems/tendencies/rules/tendencias'
import { gerarNomeAleatorio } from '@systems/names/rules/nomes'
import { STEPS, valoresDefinidos, pontosCompraMax } from '@features/character-creation/data/characterData'
...
export function useCharacterCreation() {
  ...
  const atributosComModificador = useMemo(() => {
    const modificadoresRaciais = getModificadoresRaciais(raca)
    const resultado = executarRegras({ valoresBase: atributos, modificadores: modificadoresRaciais })
    return resultado.atributosFinais
  }, [atributos, raca])

  const resultadoRegras = useMemo(() => {
    const modificadoresRaciais = getModificadoresRaciais(raca)
    return executarRegras({ valoresBase: atributos, modificadores: modificadoresRaciais })
  }, [atributos, raca])
