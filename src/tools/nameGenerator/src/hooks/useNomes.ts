...
export function useNomes() {
  ...
  const [culturaFiltro, setCulturaFiltro] =
    useState<string[]>([])

  const [sortStack, setSortStack] =
    useState<SortField[]>([])
  ...
  const estatisticas = useMemo(() => {
    ...
    for (const nome of nomes) {
      ...
      for (const cultura of culturasNome) {
        ...
        if (universal) {
          incrementarGenero(
            stats[cultura].universais,
            nome.genero
          )
        } else if (unico) {
          incrementarGenero(
            stats[cultura].unicos,
            nome.genero
          )
        } else {
          incrementarGenero(
            stats[cultura].compartilhados,
            nome.genero
          )
        }
      }
    ...
    return [...nomes]
      .filter(item => {
        ...
        const matchCultura =
          ...
          culturaFiltro.some(c =>
            item.culturas.includes(c)
          )
      ...
      .sort((a, b) => {
        for (const campo of sortStack) {
          ...
          if (campo === 'nome') {
            ...
            const aHas =
              a.culturas.length === 0 ||
              a.culturas.includes(campo)

            const bHas =
              b.culturas.length === 0 ||
              b.culturas.includes(campo)
  ...
  function toggleCultura(
    ...
    setNomes(prev =>
      prev.map(item => {
        ...
        const hasCultura =
          item.culturas.includes(cultura)

        return {
          ...item,
          culturas: hasCultura
            ? item.culturas.filter(
                c => c !== cultura
              )
  ...
  function parseAdicionar(
    texto: string
  ): Nome[] {
    const [nomesParte, configParte = ''] =
      texto.split('-')
    ...
    let genero: Nome['genero'] =
      'masculino'

    const culturas = new Set<string>()

    const culturasMap: Record<
      string,
      string
    > = {
      h: 'human',
