...
import { MetodoAtributos, metodoLabel, gerarAtributos4d6, valoresDefinidos, pontosCompraMax, calcularCusto } from '../engine/atributos'

export default function NovaHeroiPage() {
  const [nome, setNome] = useState('')
  const [metodo, setMetodo] = useState<MetodoAtributos>('4d6-baixo')
  ...
  const pontosUsados = Object.values(atributos).reduce((acc, val) => acc + calcularCusto(val), 0)
  ...
  function handleAtributoClick(campo: string) {
    if (metodo === '4d6-baixo' || metodo === 'definido') {
  ...
  return (
    <div className="page container">
      ...
      <form onSubmit={handleSubmit} className="form-heroi">
        ...
        <div className="form-group">
          ...
          <div className="metodos-grid">
            {(['4d6-baixo', 'definido', 'compra', 'livre'] as MetodoAtributos[]).map(m => (
              <button
                ...
                onClick={() => {
                  setMetodo(m)
                  if (m === '4d6-baixo') aplicarMetodo4d6()
                  else if (m === 'definido') aplicarMetodoDefinido()
                }}
              >
                {metodoLabel(m)}
              </button>
          ...
          <p className="metodo-dica">
            {metodo === '4d6-baixo' || metodo === 'definido' ? 'Clique para trocar os atributos de ordem' : 
        ...
        <div className="atributos-grid">
          {Object.entries(atributos).map(([key, value]) => (
            <div 
              key={key} 
              className={`atributo ${(metodo === '4d6-baixo' || metodo === 'definido') && atributoSelecionado === key ? 'selected' : ''}`}
              onClick={() => (metodo === '4d6-baixo' || metodo === 'definido') && handleAtributoClick(key)}
            >
              ...
              <input
                ...
                value={value}
                readOnly={metodo === '4d6-baixo' || metodo === 'definido' || metodo === 'compra'}
                onMouseDown={() => {
                  if (metodo === 'compra') handleMouseDownPontos(key)
                  else if (metodo === '4d6-baixo' || metodo === 'definido') handleAtributoClick(key)
                }}
                onMouseUp={(e) => {
                  if (metodo === 'compra') handleMouseUpPontos(key, e)
                  else if (metodo === '4d6-baixo' || metodo === 'definido') handleAtributoClick(key)
                }}
                onMouseLeave={() => {
                  if (pressTimerRef.current) {
